...
var transformsFragmentStrings = validTransforms.reduce((a, v) => __spreadProps(__spreadValues({}, a), { [v]: v + "(" }), {});
var noop = () => {
};
...
var composeTween = (tween, siblings) => {
  ...
  if (tweenCompositionType === compositionTypes.replace) {
    ...
    if (!lookupTween) {
      lookupTween = __spreadValues({}, tween);
      ...
      if (isAnim) {
        ...
        if (isFnc(a3)) {
          ...
          parsedTargetsArray.forEach((target) => {
            const staggeredChildParams = __spreadValues({}, childParams);
...
var cloneNodeProperties = (node, targetNode, newState) => {
  ...
  __spreadValues({}, node.properties);
...
var AutoLayout = class {
  ...
  animate(params = {}) {
    ...
    if (transformedLength) {
      ...
      this.transformAnimation = waapi.animate(transformed, __spreadValues({
        translate: ($el) => {
          const node = newState.getNode($el);
